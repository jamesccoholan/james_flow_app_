...
export default function PackDetails() {
  ...
  const { fetchDappiesOfPack, mintFromPack, fetchPackDetails } =
    useDappyPacks();
  ...
  const fetchDappies = async () => {
    let dappiesOfPack = await fetchDappiesOfPack(
      parseInt(packID.replace("Pack", ""))
    );
    setDappies(dappiesOfPack);
    let packDetails = await fetchPackDetails(
      parseInt(packID.replace("Pack", ""))
    );
    setPack(packDetails);
  };
  ...
  return (
    <div className="packdetails__wrapper">
      <img
        className="pack__img"
        src={`${process.env.PUBLIC_URL}/assets/${packID}.png`}
        alt="Pack"
      />
      <div className="pack__content">
        ...
        >
          ...
          ></i>{" "}
          {parseInt(pack?.price)} FUSD
        </div>
        <p>Dappies included:</p>
        <p>{dappies.map((d, i) => ` #${d} `)}</p>
      </div>
    </div>
  );
}
